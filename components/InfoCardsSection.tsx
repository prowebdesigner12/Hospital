...
const Card: React.FC<{ card: InfoCard }> = ({ card }) => (
    <a href="#" className={`flex items-center p-5 rounded-2xl w-80 lg:w-96 h-28 ${card.bgColor} shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1`}>
        <div className="flex-shrink-0 w-14 h-14 bg-white rounded-full flex items-center justify-center mr-4 shadow-inner">
            {card.icon}
        </div>
